...
namespace Demo.WindowsForms
{
    public partial class MainForm : Form
    {
        ...
        PointLatLng start;
        PointLatLng end; private List<PointLatLng> cityAreaPoint = new List<PointLatLng>();
        private GMapOverlay mGmapOverCityRouter;

        private void DrawCityArea(int i)
        {
            try
            {
                cityAreaPoint.Clear();
                mGmapOverCityRouter.Routes.Clear();
                ...
                foreach (DirectoryInfo NextFolder in dirInfo)
                {
                    if (i == 0)
                    {
                        if (NextFolder.Name == "省")
                        {
                            ...
                            {
                                if (NextFile.Name.Substring(0, NextFile.Name.IndexOf(".")) == list_cs.SelectedNode.Text)
                                {
                                    ...
                                    while (strArea.IndexOf(";") > 0)
                                    {
                                        ...
                                        tempPoint.Lat = tempPoint.Lat + OffSetLat;
                                        tempPoint.Lng = tempPoint.Lng + OffSetLon;
                                        tempPoint = EvilTransform.transform(tempPoint, MainMap.MapProvider);
                                    ...
                                    tempPoint1.Lat = tempPoint1.Lat + OffSetLat;
                                    tempPoint1.Lng = tempPoint1.Lng + OffSetLon;
                                    tempPoint1 = EvilTransform.transform(tempPoint1, MainMap.MapProvider);
                                    cityAreaPoint.Add(tempPoint1);
                                    GMapRoute r = new GMapRoute(cityAreaPoint, "Gps");//将路转换成线  
                                    r.Stroke = GMapDashStyle.DushPen1;
                                    r.Stroke.Width = 2;
                                    r.Stroke.Color = Color.Blue;
                                    mGmapOverCityRouter.Routes.Add(r);//将道路加入图层
                                    MainMap.SelectedArea1 = cityAreaPoint;
                                }
                            }
                        }
                    }
        ...
        private void list_cs_AfterSelect(object sender, TreeViewEventArgs e)
        {
            DrawCityArea(list_cs.SelectedNode.Level);
        }

        public void CS_ListAdd()
        {
            string path = System.Environment.CurrentDirectory + "/china";
            DirectoryInfo theFolder = new DirectoryInfo(path);
            DirectoryInfo[] dirInfo = theFolder.GetDirectories();
            foreach (DirectoryInfo NextFolder in dirInfo)
            {
                if (NextFolder.Name != "省")
                {
                    ...
                    t.Text = NextFolder.Name;
                    list_cs.Nodes.Add(t);
                    FileInfo[] fileInfo = NextFolder.GetFiles();
                    foreach (FileInfo NextFile in fileInfo)
                    {
                        TreeNode t1 = new TreeNode();
                        t1.Text = NextFile.Name.Substring(0, NextFile.Name.IndexOf(".")); ;
                        t.Nodes.Add(t1);
                    }
                }
            }
            foreach (var item in list_cs.Nodes)
            {
                if (((TreeNode)item).Text == "浙江")
                {
                    list_cs.SelectedNode = ((TreeNode)item);
                }
            }
        }

        public class GMapDashStyle
        {
            public static float[] dashArray = {  
        5.0f,  //线长5个像素  
        5.0f,  //间断5个像素  
        5.0f,  //线长5个像素  
        5.0f  //间断5个像素 
            ...
            public static float[] dashArray1 = {  
        3.0f,  //线长5个像素  
        2.0f,  //间断5个像素  
        3.0f,  //线长5个像素  
        2.0f  //间断5个像素 
            ...
            /// </summary>
            public static Pen Stroke = new Pen(Color.FromArgb(155, Color.MidnightBlue), 1);
            /// <summary>
            /// background color
            /// </summary>
            public static Brush Fill = new SolidBrush(Color.FromArgb(155, Color.AliceBlue));

            private static Pen _dotPen;
            /// <summary>
            ///  .... 
            /// </summary>
            public static Pen DotPen
            {
                get
                {
                    if (_dotPen == null)
                    {
                        _dotPen = new Pen(Color.FromArgb(155, Color.MidnightBlue));
                        _dotPen.DashStyle = System.Drawing.Drawing2D.DashStyle.Dot;
                    }
                    return _dotPen;
                }
            }
            ...
            /// - - - - 
            /// </summary>
            public static Pen DushPen
            {
                get
                {
                    if (_dushpen == null)
                    {
                        _dushpen = new Pen(Color.FromArgb(155, Color.MidnightBlue));
                        _dushpen.DashPattern = GMapDashStyle.dashArray;
                    }
                    return _dushpen;
                }
            }
            public static Pen DushPen1
            {
                get
                {
                    if (_dushpen == null)
                    {
                        _dushpen = new Pen(Color.FromArgb(155, Color.MidnightBlue));
                        _dushpen.DashPattern = GMapDashStyle.dashArray1;
                    }
                    return _dushpen;
                }
            }
            private static Pen _dushpen;
        ...
        public MainForm()
        {
            ...
            if (!GMapControl.IsDesignerHosted)
            {
                ...
                if (!Stuff.PingNetwork("www.amap.com"))
                {
                ...
                MainMap.Position = new PointLatLng(28, 120);
                MainMap.MinZoom = 0;
                MainMap.MaxZoom = 24;
                MainMap.Zoom = 9;
                MainMap.MouseWheelZoomType = MouseWheelZoomType.MousePositionWithoutCenter;
                mGmapOverCityRouter = new GMapOverlay();
                MainMap.Overlays.Add(mGmapOverCityRouter);
        ...
        void MainMap_MouseDown(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
            {
                ...
                if (currentMarker.IsVisible)
                {
                    ...
                    currentMarker.ToolTipText = currentMarker.Position.ToString() + "\n" + EvilTransform.transform(currentMarker.Position, MainMap.MapProvider).ToString();
        ...
        private void comboBoxMapType_DropDownClosed(object sender, EventArgs e)
        {
            ...
            if (MainMap.MapProvider.Name.Contains("Pgis"))
            {
                groupBox1.Visible = true;
                MainMap.MapProvider.RefererUrl = PgisIP.Text;
                MainMap.MapProvider.RefererPort = txPort.Text;
            }
            else
            {
                groupBox1.Visible = false;
            }
            list_cs_AfterSelect(null, null);
        }
        ...
        private void button8_Click(object sender, EventArgs e)
        {
            try
            {
                OffSetLat = double.Parse(textBoxLat.Text, CultureInfo.InvariantCulture);
                OffSetLon = double.Parse(textBoxLng.Text, CultureInfo.InvariantCulture);
        ...
        private void button11_Click(object sender, EventArgs e)
        {
            ...
            List<PointLatLng> area1 = MainMap.SelectedArea1;
            if (!area.IsEmpty || area1 != null)
            {
                Choosebox cho = new Choosebox();
                cho.ShowDialog();
                if (Choosebox.result == false)
                    return;
                int start = Choosebox.start;
                int end = Choosebox.end;
                string mapname = Choosebox.mapname;
                MainMap.CacheLocationFlieName = mapname + ".gmdb";
                DialogResult res = MessageBox.Show(string.Format("下载图层第 {0}" + " 到{1}层?", start, end), "GMap.NET", MessageBoxButtons.YesNoCancel);
                if (res == DialogResult.Yes)
                {
                    for (int i = start; i <= end; i++)
                    {
                        using (TilePrefetcher obj = new TilePrefetcher())
                        {
                            ...
                            if (!area.IsEmpty)
                            {
                                obj.Start(area, i, MainMap.MapProvider, MainMap.Manager.Mode == AccessMode.CacheOnly ? 0 : 1, MainMap.Manager.Mode == AccessMode.CacheOnly ? 0 : 1);
                            }
                            else
                            {
                                obj.Start1(area1, i, MainMap.MapProvider, MainMap.Manager.Mode == AccessMode.CacheOnly ? 0 : 1, MainMap.Manager.Mode == AccessMode.CacheOnly ? 0 : 1);
                            }
                        }
                    }
                    var path = Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData) + @"\GMap.NET\TileDBv5\en\" + mapname + ".INI";
                    IniHelper iniHelper = new IniHelper(path);
                    iniHelper.ExistINIFile();
                    iniHelper.IniWriteValue("地图配置","minZoom", start.ToString());
                    iniHelper.IniWriteValue("地图配置", "zoom", start.ToString());
                    iniHelper.IniWriteValue("地图配置", "maxZoom", end.ToString());
                    iniHelper.IniWriteValue("地图配置", "mapProvider", MainMap.MapProvider.ToString());
                    if (!area.IsEmpty)
                    {
                    ...
                    {
                        Point[] points = new Point[area1.Count];
                        for (int a = 0; a < area1.Count; a++)
                        {
                            Point p = new Point((int)area1[a].Lat, (int)area1[a].Lng);
                            points[a] = p;
                        }
            ...
            {
                MessageBox.Show("压住Alt键选择截图区域", "GMap.NET", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
            }
        }
