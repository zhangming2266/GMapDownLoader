...
namespace GMap.NET.CacheProviders
{
   ...
   using SQLiteParameter = Mono.Data.Sqlite.SqliteParameter;
    ...
    public class SQLitePureImageCache : PureImageCache
    {
        ...
        private string _fileName = "Data.gmdb";
        /// <summary>
        /// catch fileName
        /// </summary>
                                    ...
                                    using (SQLiteTransaction tr = cn2.BeginTransaction())
                                    {
                                        try
                                        {
                                            ...
                                            foreach (long id in add)
                                            {
                                                ...
                                                if (GMaps.Instance.ImPortExportStep != null)
                                                {
                                                    curStep++;
                                                    if (curStep % step == 0)
                                                    {
                                                        GMaps.Instance.ImPortExportStep((int)(curStep / step));
                                                    }
                                                }
                                            }
                                            add.Clear();
        ...
        class BatchPutImage
        {
            ...
            public GPoint pos;
            public int zoom;
            public BatchPutImage(byte[] tile1, int type1, GPoint pos1, int zoom1)
            {
                tile = tile1;
                type = type1;
                pos = pos1;
        ...
        private List<BatchPutImage> BatchPutImageData = new List<BatchPutImage>();

        public bool BatchPutImageToCatch(byte[] tile, int type, GPoint pos, int zoom)
        {
            try
            {
                if (BatchPutImageData.Count < BatchCount)
                {
                    BatchPutImageData.Add(new BatchPutImage(tile, type, pos, zoom));
                }
                else
                    return BatchUpDate();
                return true;
        ...
        {
            BatchPutImageData.Clear();
            BatchPutImage.curTime = DateTime.Now;
        }
