...
namespace GMap.NET.Internals
{
   ...
   using Thread=OpenNETCF.Threading.Thread2;
    ...
    public class EvilTransform
    {
        ...
        /// <returns></returns>
        static PointLatLng transform1(double wgLat, double wgLon)
        {
            double mgLat = 0;
            double mgLon = 0;
        ...
        /// <returns></returns>
        private static PointLatLng reTransform1(double GcLat, double GcLon)
        {
            double lontitude = GcLon * 2 - transform1(GcLat, GcLon).Lng;
            double latitude = GcLat * 2 - transform1(GcLat, GcLon).Lat;
        ...
        {
            return reTransform1(GcLat, GcLon);
        }
    ...
    internal class Core : IDisposable
    {
      ...
      public int LevelsKeepInMemmory = 1;
        ...
        public GPoint FromLatLngToLocal(PointLatLng latlng)
        {
            ...
            pLocal.X = pLocal.X + renderOffset.X - compensationOffset.X;
            pLocal.Y = pLocal.Y + renderOffset.Y - compensationOffset.Y;
        ...
        void UpdateBounds()
        {
            ...
            try
            {
                ...
                //for (long i = (int)Math.Floor(-sizeOfMapArea.Width * 3.0), countI = (int)Math.Ceiling(sizeOfMapArea.Width * 3.0); i <= countI; i++)
                //{
                //    for (long j = (int)Math.Floor(-sizeOfMapArea.Height * 3.0), countJ = (int)Math.Ceiling(sizeOfMapArea.Height * 3.0); j <= countJ; j++)
                for (long i = (int)Math.Floor(-sizeOfMapArea.Width * 1.5), countI = (int)Math.Ceiling(sizeOfMapArea.Width * 1.5); i <= countI; i++)
                {
                    for (long j = (int)Math.Floor(-sizeOfMapArea.Height * 1.0), countJ = (int)Math.Ceiling(sizeOfMapArea.Height * 1.0); j <= countJ; j++)
                    {
