...
namespace GMap.NET.Projections
{
   ...
   public class MercatorProjection : PureProjection
   {
      ...
      System.Collections.Generic.Dictionary<int, GSize> DicGSizes = new System.Collections.Generic.Dictionary<int, GSize>();
      ...
      public override GPoint FromLatLngToPixel(double lat, double lng, int zoom)
      {
         ...
         double y = 0.5 - Math.Log((1 + sinLatitude) / (1 - sinLatitude)) / (4 * Math.PI);
          if(!DicGSizes.ContainsKey(zoom))
              DicGSizes.Add(zoom,GetTileMatrixSizePixel(zoom));
         GSize s = DicGSizes[zoom];
