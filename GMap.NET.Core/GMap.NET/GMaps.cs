...
namespace GMap.NET
{
   ...
   {
       /// <summary>
       /// call back function of import and export DB files
       /// </summary>
       public Action<int> ImPortExportStep = null;
      /// <summary>
      /// tile access mode
      /// </summary>
               ...
               if(task.HasValue)
               {
                  ...
                  if(task.Value.Img != null)
                  {
                     ...
                     if((task.Value.CacheType & CacheUsage.First) == CacheUsage.First && PrimaryCache != null)
                     {
                        ...
                        PrimaryCache.BatchPutImageToCatch(task.Value.Img, task.Value.Tile.Type, task.Value.Tile.Pos, task.Value.Tile.Zoom);
                     }
      ...
      public PureImage GetImageFrom(GMapProvider provider, GPoint pos, int zoom, out Exception result)
      {
         ...
         //{
         //    PgisSLYXProvider.PgisTraslateXYZ(ref pos, ref zoom);
         //}
         try
         {
