...
namespace GMap.NET
{
   ...
   public abstract class PureProjection
   {
      ...
      public List<GPoint> GetAreaTileList1(List<PointLatLng> rect, int zoom, int padding)
      {
          List<GPoint> ret = new List<GPoint>();
          List<long> fw_x = new List<long>();
          List<long> fw_y = new List<long>();
          List<GPoint> fw = new List<GPoint>();
          foreach (var i in rect)
          {
              GPoint topLeft = FromPixelToTileXY(FromLatLngToPixel(i.Lat, i.Lng, zoom));
              if (!fw_x.Contains(topLeft.X))
              {
                  fw_x.Add(topLeft.X);
              }
              if (!fw_y.Contains(topLeft.Y))
              {
                  fw_y.Add(topLeft.Y);
              } if (!fw.Contains(topLeft))
              {
                  fw.Add(topLeft);
              }
          }
          ...
          fw_y.Sort();
          long min_X = fw_x[0];
          long max_X = fw_x[fw_x.Count - 1];
          long min_Y = fw_y[0];
          long max_Y = fw_y[fw_y.Count - 1];
          if (fw.Count < 3)
          {
          ...
          {
              ret = PolygonIsContainPoint(fw, min_X, min_Y, max_X, max_Y);
          }
          return ret;
      }

      public List<GPoint> PolygonIsContainPoint(List<GPoint> Gpoints, long min_X, long min_Y, long max_X, long max_Y)
      {
          List<GPoint> ret = new List<GPoint>();
          Point[] points = new Point[Gpoints.Count];
          for (int a = 0; a < Gpoints.Count; a++)
          {
              Point p = new Point((int)Gpoints[a].X, (int)Gpoints[a].Y);
              points[a] = p;
          }
          ...
          for (long x = min_X; x <= max_X; x++)
          {
              for (long y = min_Y; y <= max_Y; y++)
              {
                  Point inputponint = new Point((int)x, (int)y);
                  Point inputponint1 = new Point((int)x + 1, (int)y);
                  Point inputponint2 = new Point((int)x - 1, (int)y);
                  Point inputponint3 = new Point((int)x, (int)y - 1);
                  Point inputponint4 = new Point((int)x, (int)y + 1);
                  if (myRegion.IsVisible(inputponint) || myRegion.IsVisible(inputponint1) || myRegion.IsVisible(inputponint2) || myRegion.IsVisible(inputponint3) || myRegion.IsVisible(inputponint4))
                  {
                      GPoint p = new GPoint(x, y);
                      if (/*!ret.Contains(p) &&*/ p.X >= 0 && p.Y >= 0)
                      {
